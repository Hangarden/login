...
public class CommentController {
    ...
    @RequestMapping(value = { "/comments/{boardIdx}/{commentParentId}" }, method = { RequestMethod.POST }) //수정시에는 comment/{idx}로 전송
    public JsonObject registerReComment(@PathVariable(value = "boardIdx", required = false) Integer boardIdx, @PathVariable(value =  "commentParentId") Integer commentParentId, @RequestBody final CommentDTO params, HttpSession session) {
        ...
        try {
            ...
            params.setCDepth(depth); // 뎁스 주입
            boolean isRegistered = commentService.registerReComment(params); //boolean 타입 변수인 isRegistered 에는 CommentService의 registerComment의 메서드의 실행 결과를 저장한다. 실행 된다면 true 아니라면 false를 저장
            jsonObj.addProperty("result", isRegistered); //결과를 result라는 이름의 프로퍼티에 JSON 객체에 추가해서 리턴합니다.
